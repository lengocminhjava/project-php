...
function categoryAction()
{
    ...
    $list_new = search_data($list_category);
    $data = array(
        'list_category' => $list_category,
        'list_new' => $list_new
    );
...
function deleteAction()
{
    ...
    $id = (int)$_GET['id'];
    if (list_product_id($id)) {
        $error['errore'] = "Không thể xóa vì nó có khóa ngoại _pr";
        $_SESSION['errore'] = $error;
        redirect("?mod=product&controller=category&action=category");
    } else {
        if (check_parent_id_pr($id)) {
            $error['bz'] = "Không thể xóa vì còn tồn tại lớp con _pr";
            // $_SESSION['errore'] = $error;
            $_SESSION['bz'] = $error;
            redirect("?mod=product&controller=category&action=category");
        } else {
            delete($id);
            $success['okz'] = "Xóa thành công _pr";
            $_SESSION['okz'] = $success;
            redirect("?mod=product&controller=category&action=category");
        }
    }
}
